...
impl<T: Clone> Array2<T> {
    ...
    /// A Map Iterator over the values of the `Array2`.
    ///
    /// # Returns
    ///
    /// * An `Array2<U>` which transforms the elements in the Array2<T> using the transform function
    pub fn map<U: Clone>(self, transform_function: impl Fn(T) -> U) -> Array2<U> {
        // transform the data within Array2
        let data: Vec<U> = self.data.into_iter().map(transform_function).collect();
        // initialize the new Array2 data
        Array2 {
            width: self.width,
            height: self.height,
            data,
